-- Let admins configure the webhook URL and shared secret from /admin/notifications
-- instead of pasting SQL into Vault manually. The notify-lead Edge Function reads
-- the same secret through lead_notify_config(), so it always matches what the
...
begin
  ...
  if p_webhook_secret is not null and length(trim(p_webhook_secret)) > 0 then
    ...
    -- First save: generate a secret so the trigger and Edge Function stay in sync.

...
import {
  googleMapsEmbedSrc,
  BEDFORD_STUYVESANT_MAP_DEFAULT_QUERY,
  BEDFORD_STUYVESANT_POI_FILTERS,
...
export default function BedfordStuyvesantPoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? BEDFORD_STUYVESANT_POI_FILTERS.find((f) => f.id === active)?.query ?? BEDFORD_STUYVESANT_MAP_DEFAULT_QUERY
    : BEDFORD_STUYVESANT_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Bedford-Stuyvesant map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${BEDFORD_STUYVESANT_POI_FILTERS.find((f) => f.id === active)?.title} near Bedford-Stuyvesant`
        ...
        <div className="hlm-map-badge">
          ...
          <h1 className="hlm-map-title">Bedford-Stuyvesant</h1>
        </div>
      </div>
      <div className="hlm-map-filters" role="toolbar" aria-label="POI Category">
        {BEDFORD_STUYVESANT_POI_FILTERS.map((filter) => (
          <button
            key={filter.id}
            type="button"

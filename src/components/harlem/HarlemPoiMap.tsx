...
export default function HarlemPoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? HARLEM_POI_FILTERS.find((f) => f.id === active)?.query ?? HARLEM_MAP_DEFAULT_QUERY
    : HARLEM_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Harlem map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={active ? `${HARLEM_POI_FILTERS.find((f) => f.id === active)?.title} near Harlem` : "Map of Harlem"}

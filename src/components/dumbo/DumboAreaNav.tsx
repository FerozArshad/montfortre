...
import {
  ...
  DUMBO_AREA_LINKS,
  DUMBO_NEIGHBORHOODS,
} from "../../data/dumboGuide";

export default function DumboAreaNav() {
  ...
  return (
    <section
      ...
      data-screen-label="Dumbo area navigation"
      ...
      <div className="hlm-guide-nav-inner">
        ...
        <div className="hlm-guide-agent" ref={agentRef}>
          <button
            ...
            title={DUMBO_AGENT.name}
            aria-expanded={agentOpen}
            aria-haspopup="true"
            onClick={() => setAgentOpen((v) => !v)}
          >
            <img src={DUMBO_AGENT.photo} alt={DUMBO_AGENT.name} />
          </button>
          {agentOpen ? (
            <div className="hlm-guide-agent-menu" role="menu">
              <div className="hlm-guide-agent-row">
                <strong>Name:</strong> {DUMBO_AGENT.name}
              </div>
              <a className="hlm-guide-agent-row" href={DUMBO_AGENT.phoneHref}>
                <strong>Phone:</strong> {DUMBO_AGENT.phoneDisplay}
              </a>
              <a className="hlm-guide-agent-row" href={`mailto:${DUMBO_AGENT.email}`}>
                <strong>Email:</strong> {DUMBO_AGENT.email}
              </a>
              <div className="hlm-guide-agent-div" />
              {DUMBO_AGENT.socials.map((s) => (
        ...
        <label className="hlm-guide-hood">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (href && href !== "/dumbo/") window.location.assign(href);
            }}
          >
            {DUMBO_NEIGHBORHOODS.map((n) => (
        ...
        <a href="https://stanley.olridx.com/Search/Sales?idr=False#115385" target="_blank" rel="noopener noreferrer" className="hlm-guide-search">

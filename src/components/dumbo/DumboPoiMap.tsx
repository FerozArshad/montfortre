...
export default function DumboPoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? DUMBO_POI_FILTERS.find((f) => f.id === active)?.query ?? DUMBO_MAP_DEFAULT_QUERY
    : DUMBO_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Dumbo map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${DUMBO_POI_FILTERS.find((f) => f.id === active)?.title} near Dumbo`
              : "Map of Dumbo"
          }

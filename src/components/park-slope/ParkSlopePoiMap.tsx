...
import {
  googleMapsEmbedSrc,
  PARK_SLOPE_MAP_DEFAULT_QUERY,
  PARK_SLOPE_POI_FILTERS,
...
export default function ParkSlopePoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? PARK_SLOPE_POI_FILTERS.find((f) => f.id === active)?.query ?? PARK_SLOPE_MAP_DEFAULT_QUERY
    : PARK_SLOPE_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Park Slope map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${PARK_SLOPE_POI_FILTERS.find((f) => f.id === active)?.title} near Park Slope`
              : "Map of Park Slope"
          }
        ...
        <div className="hlm-map-badge">
          ...
          <h1 className="hlm-map-title">Park Slope</h1>
        </div>
      </div>
      <div className="hlm-map-filters" role="toolbar" aria-label="POI Category">
        {PARK_SLOPE_POI_FILTERS.map((filter) => (
          <button
            key={filter.id}
            type="button"

...
import { googleMapsEmbedSrc, CHELSEA_MAP_DEFAULT_QUERY, CHELSEA_POI_FILTERS } from "../../data/chelseaGuide";
...
export default function ChelseaPoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? CHELSEA_POI_FILTERS.find((f) => f.id === active)?.query ?? CHELSEA_MAP_DEFAULT_QUERY
    : CHELSEA_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Chelsea map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${CHELSEA_POI_FILTERS.find((f) => f.id === active)?.title} near Chelsea`
              : "Map of Chelsea"
          }
        ...
        <div className="hlm-map-badge">
          ...
          <h1 className="hlm-map-title">Chelsea</h1>
        </div>
      </div>
      <div className="hlm-map-filters" role="toolbar" aria-label="POI Category">
        {CHELSEA_POI_FILTERS.map((filter) => (
          <button
            key={filter.id}
            type="button"

/** Dumbo neighborhood-guide data from live WP + HTTrack capture. */

export const DUMBO_MAP_CENTER = { lat: 40.702060904128, lng: -73.986802023208 };

export const DUMBO_MAP_DEFAULT_QUERY = "Dumbo, Brooklyn, New York, NY";

export const DUMBO_POI_FILTERS = [
  { id: "hospital", title: "Hospitals", query: "hospitals near Dumbo Brooklyn New York NY" },
...
export const DUMBO_NEIGHBORHOODS = [
...
export function googleMapsEmbedSrc(query: string) {
  const { lat, lng } = DUMBO_MAP_CENTER;
  const center = `${lat},${lng}`;
  if (query === DUMBO_MAP_DEFAULT_QUERY) {

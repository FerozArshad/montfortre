...
`App.tsx` must keep `<MobileHeader />` above `<Routes>` with the 13 routes in `src/App.tsx` plus `NotMigrated`.
...
- Unmigrated live URLs (`/upper-west-side/` etc.) — stay `NotMigrated`

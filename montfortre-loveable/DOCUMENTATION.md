...
| Git / ship | Edit `montfortre-ghl-studio/`. Deploy only via overlay → `montfortre-loveable` → `smontfort/monttfortre` `main`. See `BRANCHES.md`. |
...
Repo root is `d:\montfortre\montfortre-ghl-studio` (GitHub `FerozArshad/montfortre`). Put app code under these paths only.

```
montfortre-ghl-studio/
...
- Do not commit `node_modules` or `d:\montfortre\montfortre-com\`.
...
6. Commit/push **`main`** when the owner wants GitHub (GHL) updated: overlay rewritten `montfortre-ghl-studio/` files onto `origin/main` from this `local-root` workspace — no extra folder per push, no `git checkout main` here. Do not commit `node_modules`. Never merge `local-root` into `main`.

### Reference material (not in git)

- `d:\montfortre\montfortre-com\` — HTTrack / design exports. Do not ship scraped HTML.
...
Canonical routes: `src/App.tsx`. Git folder: `d:\montfortre\montfortre-ghl-studio`.
...
## 8. Remotes + Lovable sync (short)

| Folder | Remote | Ship? |
|---|---|---|
| `montfortre-ghl-studio/` (this app) | `FerozArshad/montfortre` | Edit here. Optional local commit. **Do not push for deploy.** |
| `montfortre-loveable/` | `smontfort/monttfortre` **`main`** | **Only deploy push.** Lovable watches this repo. |
| `montfortre-com/` | — | Research only. Never ship. |

**Sync:** copy changed files from `montfortre-ghl-studio/src/...` → `montfortre-loveable/src/...` (drop the studio folder prefix) → `git pull --ff-only` then commit + push Lovable `main`. No merge between remotes.

Details: parent / this folder **`BRANCHES.md`**, **`LOVABLE-GO-LIVE.md`**, `.cursor/rules/git-main-vs-local-root.mdc`.

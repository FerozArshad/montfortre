...
import {
  googleMapsEmbedSrc,
  UWS_MAP_DEFAULT_QUERY,
  UWS_POI_FILTERS,
} from "../../data/upperWestSideGuide";
...
export default function UpperWestSidePoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? UWS_POI_FILTERS.find((f) => f.id === active)?.query ?? UWS_MAP_DEFAULT_QUERY
    : UWS_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Upper West Side map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${UWS_POI_FILTERS.find((f) => f.id === active)?.title} near Upper West Side`
        ...
        <div className="hlm-map-badge">
          ...
          <h1 className="hlm-map-title">Upper West Side</h1>
        </div>
      </div>
      <div className="hlm-map-filters" role="toolbar" aria-label="POI Category">
        {UWS_POI_FILTERS.map((filter) => (
          <button
            key={filter.id}
            type="button"

...
import {
  googleMapsEmbedSrc,
  DOWNTOWN_BROOKLYN_MAP_DEFAULT_QUERY,
  DOWNTOWN_BROOKLYN_POI_FILTERS,
...
export default function DowntownBrooklynPoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? DOWNTOWN_BROOKLYN_POI_FILTERS.find((f) => f.id === active)?.query ?? DOWNTOWN_BROOKLYN_MAP_DEFAULT_QUERY
    : DOWNTOWN_BROOKLYN_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Downtown Brooklyn map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${DOWNTOWN_BROOKLYN_POI_FILTERS.find((f) => f.id === active)?.title} near Downtown Brooklyn`
              : "Map of Downtown Brooklyn"
          }
        ...
        <div className="hlm-map-badge">
          ...
          <h1 className="hlm-map-title">Downtown Brooklyn</h1>
        </div>
      </div>
      <div className="hlm-map-filters" role="toolbar" aria-label="POI Category">
        {DOWNTOWN_BROOKLYN_POI_FILTERS.map((filter) => (
          <button
            key={filter.id}
            type="button"

...
import {
  googleMapsEmbedSrc,
  BROOKLYN_HEIGHTS_MAP_DEFAULT_QUERY,
  BROOKLYN_HEIGHTS_POI_FILTERS,
...
export default function BrooklynHeightsPoiMap() {
  const [active, setActive] = useState<string | null>(null);
  const query = active
    ? BROOKLYN_HEIGHTS_POI_FILTERS.find((f) => f.id === active)?.query ?? BROOKLYN_HEIGHTS_MAP_DEFAULT_QUERY
    : BROOKLYN_HEIGHTS_MAP_DEFAULT_QUERY;

  return (
    <section className="hlm-map" data-screen-label="Brooklyn Heights map" id="section-poi-map">
      <div className="hlm-map-frame">
        <iframe
          title={
            active
              ? `${BROOKLYN_HEIGHTS_POI_FILTERS.find((f) => f.id === active)?.title} near Brooklyn Heights`
        ...
        <div className="hlm-map-badge">
          ...
          <h1 className="hlm-map-title">Brooklyn Heights</h1>
        </div>
      </div>
      <div className="hlm-map-filters" role="toolbar" aria-label="POI Category">
        {BROOKLYN_HEIGHTS_POI_FILTERS.map((filter) => (
          <button
            key={filter.id}
            type="button"

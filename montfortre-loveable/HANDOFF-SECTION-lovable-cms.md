...
**Local workspace:** `d:\montfortre` on branch **`local-root`** (app under `montfortre-ghl-studio/`)  
...
### Env (local `montfortre-ghl-studio/.env` — gitignored)
...
## 3. Key files (local `montfortre-ghl-studio/`)
...
1. Clone Lovable repo to **sibling** (e.g. `d:\montfortre\montfortre-loveable`) — not into `local-root` as `main`.
...
| Item | Status |
|------|--------|
| Admin UI (login, dashboard, listings / pages / blog CRUD) | Done in `montfortre-ghl-studio/` |
...
| Item | Where |
|------|--------|
| Publishable URL/key | Local `montfortre-ghl-studio/.env` (gitignored); Lovable secrets UI for deploy |

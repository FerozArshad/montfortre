...
|---|---|---|
| `d:\montfortre` (Cursor workspace) | **`local-root` only** | Clone/layout: `montfortre-ghl-studio/` + `montfortre-com/` + docs. Not GHL. Not production. |
| GitHub `main` | **`main` only** | What GHL fetches. App at **repo root** (`src/`, `public/`, …). |

Work under `montfortre-ghl-studio/`. Ship **only rewritten** app files into `montfortre-loveable/` (repo root paths) and push **`smontfort/monttfortre` `main`**. Never merge the two remotes. See `.cursor/rules/git-main-vs-local-root.mdc` and `BRANCHES.md`.
...
| Folder / path | Use |
|---|---|
| `d:\montfortre\` | Cursor workspace; always branch **`local-root`** |
| `montfortre-ghl-studio/` | **The app** you edit. On GitHub `main`, these files live at **repo root** (no `montfortre-ghl-studio/` prefix) |
| `montfortre-com/` | HTTrack captures, design HTML, blog inventory notes — **local research only** |
| `.cursor/` | Local agent rules (incl. git-main-vs-local-root) — **never ship to `main`** |

### Inside `montfortre-ghl-studio/` (app — this is what GHL fetches from `main`)
...
1. **Research (local only):** HTTrack under `montfortre-com/httrack/cdn-captures/montfortre.com/<slug>/` + live page + Screaming Frog row.
...
2. Implement under montfortre-ghl-studio/ on local-root (this workspace — active working branch)
...
5. Overlay ONLY new/updated app files onto origin/main with paths rewritten:
      ...
      montfortre-ghl-studio/*.md (app docs) → *.md at repo root
   Parent of the ship commit = origin/main. Do not checkout main here.
   Do not git worktree add. Do not create d:\montfortre_main_pushN.
...
**Never:** `git checkout main` inside `d:\montfortre` · merge local-root ↔ main · force-push main · push `montfortre-com/` or `.cursor/` · push full local-root tree · create a new local folder/worktree per GHL push.
...
| **`HANDOFF-SECTION-content-asset-idx-home-pdf.md`** | Neighborhood guide restores, FAQ accordions, live `migrated-live` assets, IDX/OLR speedups, home 15-mistakes PDF, footer, Lovable ship state | Lovable `main` @ `031be8f` (stream ship `3441abf`) | **Start here for the Sept 2026 content/asset/IDX stream**; next = 253-URL DOM parity, unwired assets, home video hero |

**Git ship reminder (all streams):** implement in `montfortre-ghl-studio/` → `git pull --ff-only` in `montfortre-loveable` → overlay rewritten paths (no studio prefix) → commit + push Lovable `main` (`smontfort/monttfortre`). Never push `montfortre-com/` or force-push.

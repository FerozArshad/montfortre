...
`App.tsx` must keep `<MobileHeader />` above `<Routes>` with the 19 migrated routes in `src/App.tsx` plus `NotMigrated`.
...
- Unmigrated live URLs (`/williamsburg/` etc.) — stay `NotMigrated`

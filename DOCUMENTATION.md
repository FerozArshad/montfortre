...
| SPA sitemap (this repo) | `public/sitemap.xml` — 28 migrated URLs only |
...
Inner pages: `services-page.css`, `blog-page.css`, `stanley-page.css`, `success-page.css`, `brownstone-guide.css`, `neighborhoods-page.css`, `about-page.css`, `harlem-page.css`, `two-family-page.css`, `home-valuation-page.css`, `mortgage-calculator-page.css`, `harlem-brownstones-page.css`, `idx-page.css`, `not-migrated.css`.
...
│   ├── App.tsx                # MobileHeader ONCE, then <Routes> (28 + *)
...
│   ├── styles/                # 47 CSS files — desktop + @media in the SAME file
│   ├── hooks/
│   ├── lib/utils.ts
│   ├── lib/mortgageMath.ts
│   ├── lib/siteOrigin.ts
...
└── public/
    ...
    ├── favicon.svg
    ├── robots.txt
    └── sitemap.xml            # 28 migrated URLs only
```
...
**Migrated (28)** — also listed in `public/sitemap.xml` and `src/App.tsx`:
...
**Not migrated (on purpose)** — `NotMigrated.tsx` + `not-migrated.css`. Live WordPress `page-sitemap.xml` lists ~197 page URLs; this SPA ships **28**. Examples still live: `/prospect-heights/`, `/3-family-house-for-sale-nyc/`, listing addresses, service subpages, blog posts (`post-sitemap.xml`). **Do not invent those pages.** Nav may still link to them; they show the “not migrated” card. Pick new work from `seo-audit/live-page-sitemap-urls.txt` (`[LEFT]`).

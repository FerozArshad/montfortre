...
| SPA sitemap (this repo) | `public/sitemap.xml` — 19 migrated URLs only |
...
│   ├── App.tsx                # MobileHeader ONCE, then <Routes> (19 + *)
...
└── public/
    ...
    ├── favicon.svg
    ├── robots.txt
    └── sitemap.xml            # 19 migrated URLs only
```
...
**Migrated (19)** — also listed in `public/sitemap.xml` and `src/App.tsx`:
...
**Not migrated (on purpose)** — `NotMigrated.tsx` + `not-migrated.css`. Live WordPress `page-sitemap.xml` lists ~197 page URLs; this SPA ships **19**. Examples still live: `/williamsburg/`, `/nyc-buyers-agent-service/`, listing addresses, service subpages, blog posts (`post-sitemap.xml`). **Do not invent those pages.** Nav may still link to them; they show the “not migrated” card. Pick new work from `seo-audit/live-page-sitemap-urls.txt` (`[LEFT]`).
